    ...
    for domain_key in link_dict.keys():
        ...
        for output_url in link_dict[domain_key]:
            ...
            if domain_key == 'deviantart.com':
                output_username = deviantart_convert(output_url)
                output_data += (output_username + '\n')
            # Handle Furaffinity
            elif domain_key == 'furaffinity.net':
                output_username = furaffinity_convert(output_url)
                output_data += (output_username + '\n')
            # Handle Inkbunny
            elif domain_key == 'inkbunny.net':
                if '.php' not in output_url:
                    output_username = inkbunny_convert(output_url)
                    output_data += (output_username + '\n')
            # Handle Pixiv
            elif domain_key == 'pixiv.net':
                if 'pixiv.net/member.php' in output_url:
                    output_username = pixiv_convert(output_url)
                    output_data += (output_username + '\n')
            # Handle Ekas Portal
            elif domain_key == 'aryion.com':
                output_username = aryion_convert(output_url)
                output_data += (output_username + '\n')
            # If no handler
            else:
                output_data += (output_url + '\n')
